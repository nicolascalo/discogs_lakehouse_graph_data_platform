...
def convert_duration_to_int(df, col_name: str):
    df = (
        df.withColumn(
            f"{col_name}_sec",
            (F.regexp_replace(F.col(f"{col_name}"), "(.*\\:)|(.*\\.)", "")),
        )
        .withColumn(
            f"{col_name}_min",
            (F.regexp_replace(F.col(f"{col_name}"), "(\\:.*)|(\\..*)", "")),
        ...
        .withColumn(
            f"{col_name}_min",
            (F.col(f"{col_name}_min").cast("int")),
        )
        .withColumn(
            f"{col_name}_sec",
            (F.col(f"{col_name}_sec").cast("int")),
        )
        .withColumn(
            f"{col_name}_seconds",
            (F.col(f"{col_name}_sec") + F.col(f"{col_name}_min") * 60),
        )
        .drop(f"{col_name}", f"{col_name}_min", f"{col_name}_sec")
    )

    return df


def split_role_comments(df):
    df = df.withColumn(
        "role_comments",
        F.when(
            F.col("role").contains("["),
            F.regexp_replace(F.regexp_replace(F.col("role"), "^.*\\[", ""), "\\]", ""),
...
def create_releasesTracksExtraartists_table(df):
    ...
    df = convert_duration_to_int(df, "track_duration")
    df = split_role_comments(df).dropDuplicates(
        [
            config["TARGET_PRIMARY_KEY"],
            *config["TARGET_SECONDARY_KEYS"],
            "role",
            "role_comments",
        ]
    )
...
def create_releasesTracksArtists_table(df):
    ...
    df = convert_duration_to_int(df, "track_duration").dropDuplicates(
        [config["TARGET_PRIMARY_KEY"], *config["TARGET_SECONDARY_KEYS"], "role"]
    )
...
def create_releasesTracksSubtracksArtists_table(df):
    ...
    df = convert_duration_to_int(df, "track_duration")
    df = convert_duration_to_int(df, "subtrack_duration")
...
def create_releasesGenres_table(df):
    ...
    df = df.select(
        ...
        [config["TARGET_PRIMARY_KEY"], *config["TARGET_SECONDARY_KEYS"], "genre"]
    )
...
def create_releasesStyles_table(df):
    ...
    df = df.select(
        ...
        [config["TARGET_PRIMARY_KEY"], *config["TARGET_SECONDARY_KEYS"], "style"]
    )
...
def create_releasesRoot_table(df):
    ...
    df = (
        ...
        .withColumn(
            "released",
            F.when(
                F.col("released").contains("-"),
                F.regexp_replace(F.col("released"), "-.*", ""),
            ).otherwise(F.col("released")),
        )
        .withColumn("released", F.col("released").cast("int"))
        .withColumnRenamed("released", "release_year")
        .dropDuplicates(
            [
                config["TARGET_PRIMARY_KEY"],
                *config["TARGET_SECONDARY_KEYS"],
