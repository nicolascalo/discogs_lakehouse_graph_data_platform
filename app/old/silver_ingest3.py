...
for dump_date in n_latest_dumps_dates:
    ...
    for cfg in table_configs:
        ...
        # Build tree from top-level keep list (everything downstream included automatically)
        tree = build_keep_tree(df_bronze, cfg["keep"])

        # Select all columns from DataFrame
        df_selected = df_bronze.select(*select_tree(df_bronze, tree))

        # Compute root hash
        df_hashed = compute_root_hash(df_selected, hash_col="root_hash")
