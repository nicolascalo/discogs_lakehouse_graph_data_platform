...
def extract_required_roots(fields, explode_path=None, post_split_cfg=None):
    ...
    if post_split_cfg:
        ...
        roots.add(post_split_cfg.get("source", "element"))
        roots.add(post_split_cfg.get("alias", "element"))
...
def select_fields_with_alias(df, fields, explode_alias="element", exploded_columns=None):
    """
    exploded_columns: list of original columns that were exploded into 'element'
    """
    exploded_columns = exploded_columns or []
    ...
    for f in fields:
        ...
        # If the field source is one of the exploded columns, map to explode_alias
        if src in exploded_columns or parts[0] == explode_alias:
...
def apply_post_split_explode(df, cfg, default_explode_alias="element"):
    """
    Explodes a string column by a separator and outputs a new column.
    
    Args:
        df: Spark DataFrame
        cfg: dict containing
            - source: the column to split
            - separator: string separator (default ",")
            - alias: name of the output exploded column (optional)
        default_explode_alias: fallback alias if cfg['alias'] not provided
    Returns:
        DataFrame with a new exploded column
    """
    src = cfg["source"]
    sep = cfg.get("separator", ",")
    alias = cfg.get("alias", default_explode_alias)

    # Build regex to split safely, trimming spaces
    sep_regex = r"\s*" + re.escape(sep.strip()) + r"\s*"

    # Navigate nested fields if needed
    parts = src.split(".")
    col_expr = F.col(parts[0])
    for p in parts[1:]:
        col_expr = col_expr.getField(p)
...
def get_dtype(schema, path: str):
    ...
    for p in parts:
        if isinstance(current, StructType):
            field = next(f for f in current.fields if f.name == p)
            current = field.dataType

        elif isinstance(current, ArrayType):
            # move into element type, then re-process same path token
...
def select_tree(df, tree, prefix=""):
    ...
    for name, children in tree.items():
        ...
        if children:
            if isinstance(dtype, StructType):
                child_df = select_tree(df, children, path)
                cols.append(F.struct(*child_df).alias(name))
