...
builder = SparkSession.builder.appName("discogs-bronze-ingest") \
...
# ✅ Use standard PySpark API instead of _jsparkSession
print("Tables:", spark.catalog.listTables())

# Optional: safer way to check if a directory is a Delta table
import os

def is_delta_table(path: str):
...
for dump_date in n_latest_dumps_dates:
    ...
    for file in raw_data_file_list_dump:
        ...
        output_dir_root = os.path.join(BRONZE_DATA_DIR,file_type)
        ...
        if is_delta_table(output_dir):
            logger.info(f"{file} -> Bronze Delta table already exists, skipping")
            continue
